...
export const getTemplate = async (templateId: number) => db.Template.findOne({ where: { templateId: templateId } })
...
export const browseTemplates = async (searchParams: WhereAttributeHash) => {
	...
	if (searchParams.asArray) {
		return db.Template.findAll({ where: where })
	} else {
		return db.Template.findOne({ where: where })
	}
}
...
export const updateTemplate = async (
	...
	db.Template.findByPk(templateId, {
		transaction
	})
		.then((template) => {
			if (template == null) {
